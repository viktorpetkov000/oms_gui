function dhtmlXContainer(obj) {
	...
	this.obj.moveContentTo = function(cont) {
		...
		if (this.sb != null) {
			...
			if (cont._doOnAttachStatusBar) cont._doOnAttachStatusBar("move");
		}

		// other objects
		var objA = this.dhxcont.mainCont;
		var objB = cont.dhxcont.mainCont;
	...
	this.obj.updateNestedObjects = function() {
		...
		if (this.layout) {
			var t = this.layout._defineWindowMinDimension(this.layout.base, true);
			var m = (this._isAcc&&this.skin=="dhx_skyblue"?2:0);
			var w2 = Math.max(t[1]+m, parseInt(this.dhxcont.mainCont.style.width)+m);
			var h2 = Math.max(t[2]+m, parseInt(this.dhxcont.mainCont.style.height)+m);
			this.layoutObj.style.width = w2+"px";
			this.layoutObj.style.height = h2+"px";
	...
	*/
	this.obj.attachMenu = function(skin) {
		...
		this.menu = new dhtmlXMenuObject(menuObj.id, (skin||this.skin));
	...
	*/
	this.obj.attachToolbar = function(skin) {
		...
		this.toolbar = new dhtmlXToolbarObject(toolbarObj.id, (skin||this.skin));
	...
	*/
	...
	this.obj.adjust = function() {
		if (this.skin == "dhx_skyblue") {
			...
			if (this.toolbar) {
				if (this._isWindow || this._isLayout) {
					document.getElementById(this.toolbarId).style.height = "49px";
